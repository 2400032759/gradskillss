...
const mapping = [
    ...
    // Secondary Accents (Some purple/blue -> indigo)
    // We already have some blue-500 and blue-600 which we should probably turn to indigo-500/indigo-600 to distinguish them from the new primary blue.
    // However, since we just replaced cyan -> blue, doing blue -> indigo NOW would also change what we just replaced. 
    // Wait, let's use tokens!
];

// Phase 1: existing blue -> indigo
const mapping1 = [
...
function processFile(filePath) {
    ...
    // First shift existing blues to indigo, and emerald to blue
    mapping1.forEach(m => { content = content.replace(m.from, m.to); });

    // Then shift cyans to blue, and blacks to navy
